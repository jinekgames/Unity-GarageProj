...
float ADD_IDX(GetSurfaceData)(FragInputs input, LayerTexCoord layerTexCoord, out SurfaceData surfaceData, out float3 normalTS, out float3 bentNormalTS)
{
...
#if defined(SAMPLER_ALPHAMAP_IDX)
    color.a = SAMPLE_UVMAPPING_TEXTURE2D(ADD_IDX(_AlphaMap), ADD_ZERO_IDX(sampler_AlphaMap), ADD_IDX(layerTexCoord.base)).r;
#endif  // defined(SAMPLER_ALPHAMAP_IDX)
    ...
    // If we use roughness map, we need to convert it to perceptual smoothness
    // Note: We use the same remap as for smoothness
    float roughness = SAMPLE_UVMAPPING_TEXTURE2D(ADD_IDX(_RoughnessMap), SAMPLER_ROUGHNESSMAP_IDX, ADD_IDX(layerTexCoord.base)).r;
    surfaceData.perceptualSmoothness = 1.0 - roughness; // Convert roughness to smoothness
